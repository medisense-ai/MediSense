...
def predict(
    ...
    with torch.no_grad():
        for case_id, data in tqdm(dataloader):
            ...
            if softmax_outputs == []:
                ...
                softmax_outputs = torch.concat((softmax_outputs, 
    ...
    return {'case_id': case_ids,                                            # Mandatory - outputs should look like this
            'preds': preds,                                                 # Mandatory - outputs should look like this
            'probabilityC0': softmax_outputs[:,0],                          # Mandatory - outputs should look like this
            'probabilityC1': softmax_outputs[:,1],                          # Mandatory - outputs should look like this
            'probabilityC2': softmax_outputs[:,2],                          # Mandatory - outputs should look like this
            'probabilityC3': softmax_outputs[:,3]}                          # Mandatory - outputs should look like this
...
def arg_parse():                                                                        # Mandatory
    ...
    parser.add_argument('--test_dir', type=str, required=True,                          # Mandatory
            help='data directory containing test data, organizers set this path.')      # Mandatory
    parser.add_argument('--train_dir', type=str, required=True,                         # Mandatory
            help='data directory containing train data, organizers set this path.')     # Mandatory
    parser.add_argument('--output_dir', type=str, required=True,                        # Mandatory
            help='output directory, organizers set this path.')                         # Mandatory
    parser.add_argument('--test', action='store_true',                                  # Mandatory
            help='Test the code on a single batch.')                                    # Mandatory 
...
if __name__ == '__main__':
    ...
    # -------- Data directory --------
    dataset = datasets.MammoClassificationDataset(
        data_dir=str(TEST_DATA_DIR),
        transform=transforms.Compose([
            datasets.ResizeMammoClassification((512, 512)),
            datasets.ToTensorMammoClassification(),
            datasets.NormalizeMammoClassification(),
        ]))
    print('Preprocessing done!')
    dataloader = DataLoader(dataset, batch_size=16, shuffle=False,
                            num_workers=4)

    model_cls = models.MammoClassificationResNet18
    # !---- Your model can be anywhere within the ~/submission directory ----!
    script_dir = os.path.dirname(os.path.abspath(__file__))                         # Mandatory
    model_path = os.path.join(script_dir, 'model', f"{model_cls.__name__}V001.pth") # Mandatory
    model = model_cls(categories=4)
    ...
    #!---- Output of task 1 classification problem ----!
    out_df.to_csv(os.path.join(args.output_dir,                                     # Mandatory
                            'classification_results.csv'), index=False)             # Mandatory
    print('Dummy classification done!')
# ****************************************************************************
    # !------------- Task 2 localization problem -------------!
# ****************************************************************************
    dataset = datasets.MammoLocalizationDataset(
        data_dir=str(TEST_DATA_DIR))
    ...
    if os.path.isfile(os.path.join(args.output_dir, 'results_with_image_info.csv')):
        ...
        dfl.to_csv(os.path.join(args.output_dir, 
                                'results_with_image_info.csv'), index=False)
...
# !------------------ The expected file system structure is as: -------------------!# Mandatory
        per_image_id_output_dirs = args.output_dir + '/localization_results/{}/{}'.format(row.case_id, row.image_id)         # Mandatory
        ...
        for category in LABELS_LOCALIZATION:                                        # Mandatory
            if category == 'No_Finding':                                            # Mandatory
                ...
                mask = np.zeros((row['height'], row['width']), dtype=np.uint8)      # Mandatory
                cv2.imwrite(per_image_id_output_dirs + f'/{category}.png',          # Mandatory
                            mask, [cv2.IMWRITE_PNG_BILEVEL, 1])                     # Mandatory
        # Use predicted bounding box to generate segmentation mask exactly.
...
# !---------------- Save all category masks in .png format even zeros ---------------!                                                                  # Mandatory
            cv2.imwrite(per_image_id_output_dirs + f'/{row["category"]}.png',       # Mandatory
                            mask, [cv2.IMWRITE_PNG_BILEVEL, 1])                     # Mandatory
    
    print("Localization submission files generated successfully!")
